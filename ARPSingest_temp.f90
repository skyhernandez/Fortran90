...
!  AUTHOR: 
!
!  Michael Kevin Hernandez, email: michael.hernandez@ou.edu (06/07/2011)
!
!-----------------------------------------------------------------------
!
      PROGRAM ingest_temp_CWB
        IMPLICIT NONE
...
!----------------------------------------------------------------------
!
      SUBROUTINE readtempfile(tempFile)
        IMPLICIT NONE
...
!
        ...
        OPEN(UNIT=FileHandle, FILE=tempFile)
        DO
!
...
!
          CALL UnitConverter(station_no, station_hgt, remark1, remark2,&
               remark3, read_pres, read_hght, read_temp, read_dewp,    &
               read_Wdir, read_Wspd, NumberOfLines, read_lat, read_lon,&
...
!
      SUBROUTINE UnitConverter(station_no, station_hgt, remark1,       &
                 remark2, remark3, read_pres, read_hght, read_temp,    &
                 read_dewp, read_Wdir, read_Wspd, NumberOfLines,       &
                 read_lat, read_lon, read_year, read_month, read_day,  &
...
!
        CALL outputIntoAscii(station_no, output_station_hgt,           &
             ...
             output_hght, output_temp, output_dewp,output_Wdir,        &
             output_Wspd, OutName)
...
!             
        ...
        WRITE(fileHandle,880) "raob", centuryChar, year, month, day,   &
                              hour, minute, ".snd"
        CLOSE(fileHandle)
...
!
      SUBROUTINE outputIntoAscii(station_no, station_hgt,              &
                 NumberOfLines, lat, lon, pres, hgt, temp, dewp, Wdir, &
                 Wspd, OutName)
        IMPLICIT NONE
