...
!  AUTHOR: 
!
!  Michael Kevin Hernandez, email: michael.hernandez@ou.edu (06/10/2011)
!
!-----------------------------------------------------------------------
!
      PROGRAM ingest_shiptemp_CWB
        IMPLICIT NONE
        ...
        CALL GETARG(1,shiptempFile)
        CALL readshiptempfile(shiptempFile)
...
!----------------------------------------------------------------------
!
      SUBROUTINE readshiptempfile(shiptempFile)
        IMPLICIT NONE
...
!
        ...
        OPEN(UNIT=FileHandle, FILE=shiptempFile)
        DO
!
...
!
          CALL UnitConverter(station_no, remark1, remark2, remark3,    &
...
!----------------------------------------------------------------------
!
      SUBROUTINE UnitConverter(station_no, remark1, remark2, remark3,  &
                 read_pres, read_hght, read_temp, read_dewp, read_Wdir,&
...
!             remark2, remark3, NumberOfLines, read_Wspd, output_Wspd)
!
...
!
        CALL outputIntoAscii(station_no, NumberOfLines, output_lat,    &
             output_lon, output_pres, output_hght, output_temp,        &
             output_dewp,output_Wdir, output_Wspd, OutName)
...
!             
        ...
        WRITE(fileHandle,880) "ship", centuryChar, year, month, day,   &
                              hour, minute, ".snd"
        CLOSE(fileHandle)
...
!-----------------------------------------------------------------------
!
      SUBROUTINE outputIntoAscii(station_no, NumberOfLines, lat, lon,  &
                 pres, hgt, temp, dewp, Wdir, Wspd, OutName)
        IMPLICIT NONE
...
!-----------------------------------------------------------------------
!
      SUBROUTINE tempflags(lat, lon, year, month, day, hour, minute,   &
        ...
        DO i = 1, NumberOfLines, 1
          SELECT CASE(remark1(i))
            ...
            CASE("12")
              WRITE(*,*) "standard level within temperature            &
                          significance level"
            CASE("13")
              WRITE(*,*) "tropopause level"
            CASE("14")
              WRITE(*,*) "standard level within wind significance level"
            CASE("15")
              WRITE(*,*) "maximum wind level"
            CASE("20")
              WRITE(*,*) "standard level within temperature and wind   &
                          wind significance level"
          END SELECT
        ENDDO
